...
- **Migration Drift Resolved** (completed 2026-08-25):
  - The migrations were never broken — they ran clean on a fresh DB first try. **The dev database was the divergent one**, and it was masking two production bugs.
  - **Missing unique constraint (real bug, fixed)**: dev had `UNIQUE (rfp_id, vendor_id)` on `rfp_vendors` but the migrations did not. Sequelize `sync()` generates it automatically for `belongsToMany` through-tables, so dev had it by accident. `rfpController.js:153` uses `RfpVendor.findOrCreate()`, which **relies on that constraint to be race-safe** — production would have silently accepted duplicate rows under concurrent sends. Added `20260825000001-add-rfp-vendors-unique-constraint.js` (uses `IF NOT EXISTS`, so it is a no-op where sync already created it).
  - **Dev was more permissive than production**: 8 FK columns were nullable in dev but `NOT NULL` in migrations. All 8 models declare `allowNull: false`, so the migrations matched intent. Cause: the association definitions in `models/index.js` specify `foreignKey: 'rfp_id'` without `allowNull`, and Sequelize merges that over the stricter model attribute during sync. Dev was also missing 23 indexes.
  - **Dev reconciled** (non-destructive; verified 0 NULL rows in all 8 columns first): applied the 8 `NOT NULL` constraints + 23 missing indexes, then baselined `SequelizeMeta` with all 6 migrations. Dev schema now matches a freshly-migrated DB **exactly**: 130 identical columns, 40 identical indexes, `migrate:status` shows 6 × `up`.
...
- **RfpList RBAC Gating** (completed 2026-08-25):
  ...
  - Verified in-browser both ways: viewer sees none of the three and gets a clean "No RFPs found." empty state; admin still sees Create plus View/Delete per row. 62 frontend tests pass.
  - Watch out: `useAuth()` throws outside an `AuthProvider` (`AuthContext.js:70`), so adding it to a page can break tests that render it standalone.

## Dev Database State (as of 2026-08-25)
- E2E test data was purged: 503 rows removed across 13 tables. All content (RFPs, documents, vendors, proposals, chats, notifications, embeddings) was e2e-generated — every RFP carried the identical title from spec 03's prompt and every vendor matched the `dell_`/`hp_` patterns.
- **Note**: e2e-created content is owned by `admin@company.com` (the account the specs log in as), not by the generated `viewer_*`/`manager_*` users — so test data cannot be scoped by owner. Scope by content pattern instead.
- Preserved: 5 real accounts (`admin@company.com`, `admin@rfp.com`, `abhisheksinghmuz1999@gmail.com`, `asthakumarsingh8051@gmail.com`, `abhissshe@cerotechnology.com`) and 3 chat conversations (14 messages) owned by the real non-admin users.
- E2E admin credentials (used by specs 01–07): `admin@company.com` / `SecurePass123`.
...
- **2026-08-25 (session 10):** Ran the full stack locally and found the job queue had been silently disabled since it was written — `jobQueue.js` targeted the pg-boss v9 API against the installed v12.18.2, so every AI operation fell back to synchronous processing. Migrated to the v12 API (6 breaking changes; the array-based `work()` handler is the dangerous one). Then ran the Playwright e2e suite for the first time: 12/16 passed, and the 4 failures traced to a wrong selector in `02_user_management.spec.js` cascading through worker restarts. Fixed both. 563 tests (501 backend + 62 frontend) + 16 e2e, all pass. Work merged to `main` and pushed. Then fixed the migration drift: proved the migrations run clean on a fresh DB, found the dev database was the divergent one, added a missing `rfp_vendors` unique constraint that `findOrCreate` depends on, reconciled dev to match migrations exactly, and hardened `sync-db.js` against re-introducing the drift. Re-ran e2e specs 03–06 against the tightened schema (5 passed, inserts confirmed at row level), then purged 503 rows of e2e test data from the dev DB. See **Dev Database State** and **Known Open Issues** above.
